...
keys = [
  ...
  Key([mod,"mod1"], "d", lazy.spawn(home+'/.config/i3/scripts/dmenu_run_history\
  ...
  Key([mod], "n", lazy.spawn('viewnior')),
  Key([mod], "b", lazy.spawn('code')),
  ...
  Key([mod], "Print", 
    lazy.spawn('flameshot screen -n 0 -p /f/screenshots/')),
  Key([mod,"shift"], "Print", 
    lazy.spawn('elameshot screen -n 1 -p /f/screenshots/')),

  Key([mod,"control"], "c", lazy.spawn('shutter -s')),
  Key([mod,"control"], "Print", lazy.spawn('shutter -w')),
...
floating_layout = layout.Floating(float_rules=[
  *layout.Floating.default_float_rules,
  ...
  Match(role="browser-window",wm_class="Skype")
...
def init_screens():
  return [
    Screen(top=bar.Bar(widgets=init_wdgets_screen1(), size=26)),
